...
const STARTERS = [
  {
    ...
    q: 'Research this company for Aerpolice fit — give me company summary, ICP score, stakeholders to target, signals, and a cold outreach message:',
  },
  {
    icon: Users,
  ...
  },
  {
    ...
    q: 'Write cold outreach for Aerpolice targeting this company — LinkedIn connection note (under 300 chars), LinkedIn DM, and a cold email. Focus on discovery, not pitching:',
  },
  {
    icon: AlertTriangle,
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function AerpoliceCopilotPage() {
  ...
  const newSession = () => {
    ...
    setShowSessions(false); toast.success('New Aerpolice BD chat')
  }
  ...
  const send = useCallback(async (text: string) => {
    ...
    try {
      const res = await fetch('/api/ai/aerpolice-copilot', {
      ...
      if (!sessionId && data.session_id) {
        ...
        // Load immediately (title already has [Aerpolice] prefix from creation)
          ...
          <div style={{ position: 'relative' }}>
            ...
            {showSessions && (
              <div style={{ position: 'absolute', right: 0, top: '110%', zIndex: 50, width: 300, maxHeight: 380, overflowY: 'auto', background: 'rgb(14,15,24)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 6, boxShadow: '0 20px 50px rgba(0,0,0,0.7)' }}>
                <div style={{ padding: '6px 8px 4px', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Aerpolice BD sessions</div>
                {sessions.length === 0 ? (
                  ...
                  <button key={s.id} onClick={() => loadSession(s)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 10px', borderRadius: 8, border: 'none', background: sessionId === s.id ? C.primaryDim : 'transparent', color: 'white', cursor: 'pointer', marginBottom: 2 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: sessionId === s.id ? C.primaryText : 'white' }}>{s.title.replace('[Aerpolice] ', '')}</div>
              ...
              <div style={{ maxWidth: 680, margin: '0 auto', paddingTop: 16 }}>
                ...
                <div style={{ borderRadius: 16, padding: '20px 22px', background: `linear-gradient(135deg, rgba(6,182,212,0.08), rgba(8,145,178,0.04))`, border: `1px solid ${C.primaryBorder}`, marginBottom: 28, boxShadow: C.glow }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 700, color: 'white', marginBottom: 5 }}>Aerpolice BD Co-Pilot</div>
                      ...
                      <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 8, background: 'rgba(0,0,0,0.3)', borderLeft: `3px solid ${C.primary}`, fontSize: 12, color: 'rgb(180,185,210)', fontStyle: 'italic', lineHeight: 1.55 }}>
                        "When an AI agent can move money, one wrong call cannot be undone. Aerpolice is the gate that determines what the agent is allowed to do before it acts."
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div style={{ maxWidth: 780, margin: '0 auto' }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', borderRadius: 14, border: `1px solid ${thinking ? C.primaryBorder : 'rgba(255,255,255,0.1)'}`, background: 'rgba(255,255,255,0.03)', padding: 7, transition: 'border-color 0.2s' }}>
                <textarea
                  ...
                  placeholder="Paste a company, LinkedIn URL, email thread, meeting notes, or ask anything about Aerpolice BD…"
                  rows={1}
