...
const STARTERS = [
  {
    ...
    q: 'Research this company for Aergap Web2 fit — what does the agent actually do, governance risk, Aergap capability match, decision-maker, and outreach angle:',
  },
  {
    icon: AlertTriangle,
    label: 'Analyze governance risk',
    q: 'Analyze the governance risk for this company\'s AI agents. What could go wrong, who is accountable, and which Aergap capability (Identity / Policy / Execution Gate / Audit Trail) solves the specific problem:',
  },
  {
    icon: FileText,
  ...
  },
  {
    ...
    q: 'Who at this company should I contact first for Aergap governance? Map the stakeholders, identify the most likely buyer and champion, and explain why they care about agent governance specifically:',
  },
  {
    icon: ClipboardList,
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function AergapWeb2CopilotPage() {
  ...
  const send = useCallback(async (text: string) => {
    ...
    try {
      const res = await fetch('/api/ai/aergap-web2-copilot', {
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
              ...
              <div style={{ maxWidth: 700, margin: '0 auto', paddingTop: 16 }}>
                ...
                <div style={{ borderRadius: 16, padding: '20px 22px', background: `linear-gradient(135deg, rgba(249,115,22,0.08), rgba(251,191,36,0.04))`, border: `1px solid ${C.primaryBorder}`, marginBottom: 28, boxShadow: C.glow }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 700, color: 'white', marginBottom: 5 }}>Aergap Web2 Co-Pilot</div>
                      <div style={{ fontSize: 12.5, color: 'rgb(150,158,190)', lineHeight: 1.65 }}>
                        Identifies Web2 companies where <strong style={{ color: 'white' }}>autonomous AI agents take real-world actions</strong> — and where Aergap's governance layer (identity, policy, execution gate, audit trail) solves a specific compliance, security, or operational risk.
                      </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 24 }}>
                  {SECTORS.map(s => (
                    <button key={s.label} onClick={() => {
                      setInput(`Research the top Aergap prospects in the ${s.label} category — which companies have the highest governance risk, what do their agents actually do, and who should I contact first?`)
        ...
        <div className={cn('hidden lg:flex')} style={{ width: 220, flexShrink: 0, borderLeft: '1px solid rgba(255,255,255,0.06)', padding: '20px 16px', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: 'rgb(90,97,125)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Aergap capabilities</div>
