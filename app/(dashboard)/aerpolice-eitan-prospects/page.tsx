...
import {
  AERPOLICE_EITAN_PROSPECTS, AERPOLICE_EITAN_SEGMENTS, AERPOLICE_EITAN_ACCOUNT_COUNT,
  AERPOLICE_LATER_MONITORING, type AerpoliceEitanProspect,
} from '@/lib/aerpolice-eitan-prospects'
...
export default function AerpoliceEitanProspectsPage() {
  ...
  useEffect(() => {
    const names = AERPOLICE_EITAN_PROSPECTS.map(c => c.company)
    getClient()
      .from('leads')
  ...
  const filtered = useMemo(() => AERPOLICE_EITAN_PROSPECTS.filter(c => {
  ...
  const leadFieldsFor = (c: AerpoliceEitanProspect) => {
  ...
  const addToPipeline = async (c: AerpoliceEitanProspect) => {
    setAdding(c.id)
    try {
  ...
  const tier1 = AERPOLICE_EITAN_PROSPECTS.filter(c => c.tier === 'Tier 1').length
    ...
    <div className="fade-in">
      ...
      <div className="page-header flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-[18px] font-bold text-white tracking-tight flex items-center gap-2">
            <KeyRound size={18} style={{ color: '#22d3ee' }} /> Aerpolice — Eitan-Qualified Prospects
          </h1>
          <p className="text-[12px] mt-1 font-medium" style={{ color: 'rgb(100,106,135)' }}>
            {AERPOLICE_EITAN_PROSPECTS.length} prospects · {AERPOLICE_EITAN_ACCOUNT_COUNT} unique accounts · wallet-key + irreversible-action gates only, per current Aerpolice Approach
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            onClick={() => {
              ...
              a.download = 'aerpolice-eitan-prospects.csv'
              a.click()
            }}
      ...
      <div style={{ padding: 'clamp(14px, 4vw, 20px) clamp(16px, 5vw, 36px)' }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            { label: 'Total Prospects', value: AERPOLICE_EITAN_PROSPECTS.length, color: '#22d3ee' },
            { label: 'Tier 1', value: tier1, color: '#34d399' },
            { label: 'Segments', value: AERPOLICE_EITAN_SEGMENTS.length - 1, color: '#38bdf8' },
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {AERPOLICE_EITAN_SEGMENTS.map(seg => (
