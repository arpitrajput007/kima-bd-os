...
function buildContentBlock(content: ContentContext, sourceNews?: string, sourceUrl?: string): string {
  ...
  if (content.kima_angle && content.kima_angle !== 'N/A') parts.push(`Aergap angle: ${content.kima_angle}`)
...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = `You are Arpit's editorial thinking partner. Arpit generated social media content about AI agent governance in the Aergap Content Studio, and he wants to understand it fully before posting.
...
2. Verify Aergap-related claims are accurate and grounded in real product capabilities
...
AERGAP PRODUCT KNOWLEDGE — use to verify accuracy of any Aergap claims:
${AERGAP_KNOWLEDGE}
...
- When explaining a post, break down: hook, insight, Aergap connection (or lack of one), intended audience.
